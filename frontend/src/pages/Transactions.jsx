...
export default function Transactions() {
  ...
  const [form, setForm] = useState({ type: 'expense', amount: '', category: '', description: '', transactionDate: new Date().toISOString().split('T')[0], paymentMethod: 'cash' });
  ...
  const openCreate = () => {
    setForm({ type: 'expense', amount: '', category: '', description: '', transactionDate: new Date().toISOString().split('T')[0], paymentMethod: 'cash' });
    setErrors({});
    setModal({ open: true, mode: 'create', data: null });
  };

  const openEdit = (tx) => {
    setForm({ type: tx.type, amount: tx.amount, category: tx.category, description: tx.description || '', transactionDate: tx.transactionDate?.split('T')[0] || '', paymentMethod: tx.paymentMethod || 'cash' });
    setErrors({});
